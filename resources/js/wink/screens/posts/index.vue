    ...
    <div class="container">
        <div class="mb-10 flex items-center">
            ...
            <filters @showing="focusSearchInput" :is-filtered="isFiltered" class="text-sm">
                <input type="text" class="input mt-0 w-full pb-2 border-b border-very-light" placeholder="Search..." v-model="searchQuery" ref="searchInput">

                <div class="flex items-center justify-between mt-5">
                    <span>Status</span>
                    <select name="status" class="border border-lighter rounded w-3/5 focus:outline-none appearance-none py-1 px-3" v-model="filters.status">
                ...
                <div class="flex items-center justify-between mt-3">
                    <span>Author</span>
                    <select name="author" class="border border-lighter rounded w-3/5 focus:outline-none appearance-none py-1 px-3" v-model="filters.author_id">
                        <option value="">All</option>
                ...
                <div class="flex items-center justify-between mt-3">
                    <span>Tag</span>
                    <select name="tag" class="border border-lighter rounded w-3/5 focus:outline-none appearance-none py-1 px-3" v-model="filters.tag_id">
                        <option value="">All</option>
                        <option v-for="tag in tags" :value="tag.id">{{tag.name}}</option>
                    </select>
                </div>

                <button v-if="isFiltered" @click.prevent="clearFilters" class="btn-sm btn-light w-full mt-5">Reset
                </button>
            </filters>
        </div>
